...
#include "sdk/android/src/jni/android_media_decoder.h"
...
MediaCodecVideoDecoder::MediaCodecVideoDecoder(JNIEnv* jni,
  ...
  rtc::Thread::Current()->AllowInvokesToThread(codec_thread_.get());
...
int32_t MediaCodecVideoDecoder::InitDecode(const VideoCodec* inst,
  ...
  // Call Java init.
  return codec_thread_->Invoke<int32_t>(RTC_FROM_HERE, [this] {
...
int32_t MediaCodecVideoDecoder::Decode(
  ...
  if ((inputImage._encodedWidth * inputImage._encodedHeight > 0) &&
    ...
    if (use_surface_ &&
      ...
      ret = codec_thread_->Invoke<int32_t>(RTC_FROM_HERE, [this] {
        return ResetDecodeOnCodecThread();
      });
    } else {
      // Hard codec reset.
  ...
  if (uid_ != inputImage.user_id_) {
     uid_ = inputImage.user_id_;
  }

  return codec_thread_->Invoke<int32_t>(RTC_FROM_HERE, [this, inputImage] {
    return DecodeOnCodecThread(inputImage);
  });
}
...
bool MediaCodecVideoDecoder::DeliverPendingOutputs(JNIEnv* jni,
  ...
  if (use_surface_) {
    ...
    rtc::scoped_refptr<I420Buffer> i420_buffer =
        decoded_frame_pool_.CreateI420Buffer(width, height);
  ...
  if (frames_decoded_ == 1) {
    zorro::GetCallManagerPrivate()->OnFirstVideoFrameDecoded(uid_);
  }

...
#include "sdk/android/src/jni/android_media_encoder.h"
...
#if RTC_DCHECK_IS_ON
  webrtc::MutexLock lock(&inited_crit_);
  RTC_DCHECK(!inited_);
#endif
}
...
int32_t MediaCodecVideoEncoder::InitEncodeInternal(int width,
  ...
  const bool encode_status = Java_MediaCodecVideoEncoder_initEncode(
      jni, j_media_codec_video_encoder_, j_video_codec_enum, (int)profile_, width,
  ...
  while (true) {
    ...
    if (callback_) {
      ...
      if (codec_type == kVideoCodecVP8 || codec_type == kVideoCodecVP9) {
        ...
        h264_bitstream_parser_.ParseBitstream(*image);
      ...
      callback_result = callback_->OnEncodedImage(*image, &info);
    }
